...
- Golden tests: `lib/test/golden/` (`CodeGen/`, `Command/`, `Strap/`,
  `Evaluate/`, `LSP/`, `State/`, `DocTest/`). Regenerate with
  `./test.sh lib-tests -p '<pattern>' --golden-reset` only after the generator
  is fixed and the new output is reviewed. Never hand-edit golden outputs.
...
These hold for every change you make to this tree.

- Codegen order is fixed: fix the emitter, then run a scoped
  `--golden-reset`, then read the golden diff. Never hand-edit a golden output
  file. Never run `--golden-reset` before the emitter fix compiles.
- Never add special `Expression` ADT constructors for interfaces. Interfaces
  resolve through the instance map.
- A fix without a test that pins it is half a fix. Write the pinning test.
- Run filtered tests only. The unfiltered suite belongs to the final gate.
